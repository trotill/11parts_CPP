...
static struct usbentry *parse_devlist(DIR *d)
{
	...
	//printf("found name %s vid %04x pid %04x\n",e->d_name,dev.vendor_id,dev.product_id);
			...
			const char *product)
{
	...
	//printf("bus %04x dev %04x vid %04x pid %04x product %s\n",*bus,*dev,*vid,*pid,(product!=NULL)?product:"");
	...
	{
		//printf("ex found name %s vid %04x pid %04x\n",e->product_name,e->vendor_id,e->product_id);
